...
The core issue is OLTP and OLAP sharing the same PostgreSQL — analytical queries compete directly with incoming writes. At scale, the storage layer would move to BigQuery/Snowflake or an open-source Lakehouse (Iceberg on GCS/S3), the Python consumer would be replaced by a distributed processor (Dataflow or Spark), and the SQL views would become dbt models. Kafka stays as the central event bus — multiple consumer groups read the same topics independently, so Kafka Connect can simultaneously archive raw events to GCS/S3 in Parquet without touching the Python consumer at all. On the producer side, Schema Registry would enforce event contracts before data enters Kafka, making malformed messages impossible rather than just catchable — which also makes the DLQ mostly redundant.
